...
// QwenReRanker 类的实现
public class CustomReRanker extends BaseReRanker<CustomRerankerConfig> {

    HttpClient httpClient = new HttpClient();

    public CustomReRanker(CustomRerankerConfig config) {
        super(config);
    }

    @Override
    public ReRankerResponse reranker(ReRankerRequest request) {
        CustomRerankerConfig config = getConfig();
